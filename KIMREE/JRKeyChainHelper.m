...
@implementation JRKeyChainHelper
...
+ (NSString *) getUserTypeWithService:(NSString *) userType{
    NSString *ret = nil;
    NSMutableDictionary *keychainQuery = [self getKeyChainQuery:userType];
    [keychainQuery setObject:(id)kCFBooleanTrue forKey:(id)kSecReturnData];
    [keychainQuery setObject:(id)kSecMatchLimitOne forKey:(id)kSecMatchLimit];
    CFDataRef keyData = NULL;
    if (SecItemCopyMatching((CFDictionaryRef)keychainQuery, (CFTypeRef *)&keyData) == noErr)
    {
        ...
        @catch (NSException *e)
        {
            NSLog(@"Unarchive of %@ failed: %@", userType, e);
        }
        @finally
        {
        }
...
+ (void)saveUserType:(UserType) userType userTypeService:(NSString *) typeService;{
    NSMutableDictionary *keychainQuery = [self getKeyChainQuery:typeService];
    SecItemDelete((CFDictionaryRef)keychainQuery);
    [keychainQuery setObject:[NSKeyedArchiver archivedDataWithRootObject:[NSString stringWithFormat:@"%lu", userType]] forKey:(id)kSecValueData];
    SecItemAdd((CFDictionaryRef)keychainQuery, NULL);
}
